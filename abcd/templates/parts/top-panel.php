...
// WP custom header
/* mods
 *  25May zig, dont show top_div if nothing in there.
 *  15May16 zig, make phone in header an a-tag with href="tel"
 */
...
<header class="top_panel_wrap bg_tint_<?php echo esc_attr($top_panel_style); ?>" <?php echo ($header_style); ?>>
   ...
   <?php 
    $show_search = (ancora_get_custom_option('show_search') == "yes");
    $show_contact_info = (ancora_get_custom_option('show_contact_info') == "yes");
    $show_contact_info = false;
    $show_disclaimer = (ancora_get_custom_option('disclaimer') == "");

    ?>
    <?php if ( $show_search || $show_contact_info || $show_disclaimer) { ?>
    ...
    <?php }  /* end if for showing top bar */ ?>
    ...
    <div class="menu_main_wrap logo_<?php _e(esc_attr(ancora_get_custom_option('logo_align'))); ?><?php _e(($ANCORA_GLOBALS['logo_text'] ? ' with_text' : '')); ?>">
        <div class="content_wrap clearfix display_none">
            ...
            <div class="logo">
                ...
                <div class="inline image side-right marg_top_2em abcd-top-phone-wrap">
                    <?php
                        if(ancora_get_custom_option('show_number_block') == 'yes') {
                            ?>
                            <div class="inline">
                                <img class="abcd-top-phone-img" src="<?php
                                    $img1 = ancora_get_custom_option('number_image');
                                    if(empty($img1))  $img1 = ancora_get_file_url('skins/' . ($theme_skin) . '/images/phone.jpg');
                                    _e($img1);
                                ?>" alt="">
                                <div class="side-right marg_null marg_top abcd-top-phone-text">
                                    <?php $phoneno = trim(ancora_get_custom_option('contact_phone')); ?>
                                    <h4><a href="tel:<?php echo $phoneno; ?>"><?php _e(force_balance_tags($phoneno)); ?></a></h4>
                                    <span class=""><?php _e(ancora_get_custom_option('text_under_number_title')); ?></span>
                                </div>
                            </div>
